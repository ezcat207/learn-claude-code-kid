#!/usr/bin/env python3
"""
s10_team_protocols_ollama.py - Team Protocols (Ollama version)

Same shutdown + plan approval protocols as s10_team_protocols.py
but uses Ollama via its OpenAI-compatible API.

    Shutdown FSM: pending -> approved | rejected
    Plan approval FSM: pending -> approved | rejected
    Both use the same request_id correlation pattern.
...
class MessageBus:
    ...
    def broadcast(self, sender: str, content: str, teammates: list) -> str:
        count = sum(1 for name in teammates if name != sender
                    and not self.send(sender, name, content, "broadcast"))
...
# -- TeammateManager with shutdown + plan approval, OpenAI format --
...
TOOLS = [
    ...
    {"type": "function", "function": {"name": "shutdown_request", "description": "Request a teammate to shut down gracefully.",
        "parameters": {"type": "object", "properties": {"teammate": {"type": "string"}}, "required": ["teammate"]}}},
    {"type": "function", "function": {"name": "shutdown_response", "description": "Check the status of a shutdown request.",
...
if __name__ == "__main__":
    ...
    while True:
        try:
            query = input("\033[36ms10-ollama >> \033[0m")
